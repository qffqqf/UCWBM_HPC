...
N_UC_x = 15;% Amount of UCs in X-direction
N_UC_y = 15;% Amount of UCs in Y-direction
...
response_UC_x = 10; % Which UC in the x-direction
response_UC_y = 10; % Which UC in the y-direction
...
uz = zeros(size(omega_range));
opts1.SYM = false;
opts2.SYM = true;

%% WBM frequency loop
tic
for iWave = 1:numel(omega_range)
    iWave
    omega = omega_range(iWave);
    ...
    bderror = norm(K_wbm*c_wbm-F_wbm);
    %% Reconstruct
    ...
    uz(iWave) = reconstruct_value(resp_loc, lambdas_all, phases_all, ...
...
timing = toc;
uz_wbm = uz;
save([savename,'.mat'],'freq','uz_wbm','timing');
