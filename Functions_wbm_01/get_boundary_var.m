function modes_bd = get_boundary_var(N_UC_x, N_UC_y, lambdas, phases, ...
                                           bloch_modes, UC_dofs)
nWave = size(lambdas,1);
%% Get local modes
modes_local_B = bloch_modes([UC_dofs.BL, UC_dofs.B],:);
modes_local_T = bloch_modes([UC_dofs.T, UC_dofs.TR],:);
modes_local_L = bloch_modes([UC_dofs.L, UC_dofs.TL],:);
modes_local_R = bloch_modes([UC_dofs.BR, UC_dofs.R],:);
%% Get global modes
for iWave = 1:nWave
    ...
    index = [phase_x: N_UC_x-1+phase_x]';
    prop_T = (lambda_x.^index) * (lambda_y^(N_UC_y-1+phase_y));
    ...
    index = [phase_y:N_UC_y-1+phase_y]';
    prop_R = (lambda_y.^index) * (lambda_x^(N_UC_x-1+phase_x));
